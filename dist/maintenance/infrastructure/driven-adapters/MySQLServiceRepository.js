...
class MySQLServiceRepository {
    ...
    async findAll() {
        ...
        return rows.map((r) => new Service_1.Service(r.id, r.tipo_servicio, r.descripcion, r.num_cliente, r.num_bicicleta, r.precio_base, r.precio_total, r.costo_piezas, r.fecha_ingreso, r.estado, r.mecanico_id, r.fecha_entrega));
    }
    async findById(id) {
        ...
        if (rows.length === 0)
            return null;
        const r = rows[0];
        return new Service_1.Service(r.id, r.tipo_servicio, r.descripcion, r.num_cliente, r.num_bicicleta, r.precio_base, r.precio_total, r.costo_piezas, r.fecha_ingreso, r.estado, r.mecanico_id, r.fecha_entrega);
    }
    async save(s) {
    ...
    async delete(id) {
        const [result] = await db_1.pool.query("DELETE FROM ventas WHERE id = ?", [id]);
        return result.affectedRows > 0;
    }
    async updateEstado(id, estado) {
        await db_1.pool.query("UPDATE servicios SET estado = ? WHERE id = ?", [estado, id]);
    }
